...
def kuramoto(t, y, N, A, D, n_W, w):
    ...
    for i in range(N):
        ...
        '''
        a13 = (-1/4)*((a[0]*a[1]*a[2])/((a[0]**2+a[1]**2)**(3/2)+a[1]*a[2]**2))
        a12 = a13*(((a[0]**2+a[1]**2)**(1/2))/a[2])
        a11 = 1+a12*(a[0]/a[1])
        a23 = (-1/4)*((a[0]*a[1]*a[2])/((a[0]**2+a[1]**2)**(3/2)+a[0]*a[2]**2))
        a21 = a23*(((a[1]**2+a[0]**2)**(1/2))/a[2])
        a22 = 1+a21*(a[1]/a[0])        
        a31 = (-1/4)*((a[2]*a[1])/(a[2]*(a[0]**2+a[1]**2)**(1/2)+a[0]*a[1]+a[1]**2))
